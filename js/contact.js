...
let firstInputsGroup = document.querySelectorAll(
  ".content form input:not(.subj)"
);
let lastInputsGroup = document.querySelectorAll(
  ".content form .subj, .content form textarea"
);
let firstSpansGroup = document.querySelectorAll(
  ".content form input:not(.subj) + span"
);
let lastSpansGroup = document.querySelectorAll(
  ".content form .subj + span, .content form textarea + span"
);
...
lastInputsGroup.forEach((input) => {
  input.addEventListener("focus", () => {
    ...
    else input.nextElementSibling.classList.add("active-lg-input");
  });
  input.addEventListener("blur", () => {
    if (input.value.length <= 0) {
      if (input.tagName == "TEXTAREA")
        document
          .querySelector(".textarea-span")
          .classList.remove("active-lg-input");
      else input.nextElementSibling.classList.remove("active-lg-input");
    }
  });
});
// Add & Remove Active class for span when clicking on the span
