...
let mediumScreen = window.matchMedia(
...
function createIndicators() {
  if (smallScreen) {
    ...
    for (
      ...
      i > Math.trunc(testimonials.length / 2);
      i--
    ) {
...
moveToNextTestimonials = () => {
  ...
  if (smallScreen) {
    if (indx >= testimonials.length - 1) return;
      ...
      //Move to the next
      indx++;
      testContainer.style.transition =
        "all 1s cubic-bezier(0.39, 0.58, 0.57, 1) 0s";
      testContainer.style.transform = `translateX(${-100 * indx}%)`;
      //Move active class to the next indicator
      document
        .querySelector("#testimonials .testi-indicators .active")
        .classList.remove("active");
      if (indx == 5)
        //Active the first indicator because we reached to the end(first cloned nodes)
        document
          .querySelector(`#testimonials .testi-indicators li`)
          .classList.add("active");
      else
        document
          .querySelector(
            `#testimonials .testi-indicators li:nth-child(${indx})`
          )
          .classList.add("active");
    }
  } else if (mediumScreen) {
    if (indx >= testimonials.length - 2) return;
      ...
      if (swap == 5)
        ...
        document
          .querySelector(
            `#testimonials .testi-indicators li:nth-child(${swap})`
          )
          .classList.add("active");
    }
  } else {
  ...
  if (indx == 0) return;
    ...
    if (smallScreen) {
      ...
      if (indx == 5)
        ...
        document
          .querySelector("#testimonials .testi-indicators li:last-child")
          .classList.add("active");
      else
        document
          .querySelector(
...
testContainer.addEventListener("transitionend", () => {
  if (smallScreen) {
    ...
    if (testimonials[indx].id == firstClone.id) {
      ...
      indx = 1;
      testContainer.style.transform = `translateX(${-100 * indx}%)`;
    } else if (testimonials[indx].id == lastClone.id) {
      //jumped to the beginning of testimonials
      //To make the movement of the reset invisible
      testContainer.style.transition = "none";
      //Reset testimonials arrangement
...
let spansInputs = document.querySelectorAll(
  "#book form input[type='text']+span,#book form input[type='email']+span"
);
